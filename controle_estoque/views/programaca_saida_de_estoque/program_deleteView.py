...
class ProgramaEstoque_DeleteView(DeleteView):
    ...
    def get_context_data(self, **kwargs):
        dados = ProgramacaoSaidaEstoque.objects.filter(pk=self.kwargs['pk'])
        for d in dados:
            dados = f'{d.data_agendamento} <=> {d.escola_beneficiaria}'
        ...
        context["title"] = 'Deletar Programação de Saída de Estoque'
